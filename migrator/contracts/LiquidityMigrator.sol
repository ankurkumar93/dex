...
contract LiquidityMigrator {
    ...
    function deposit(uint amount) external {
        require(migrationsDone == false, 'Migration is done before');
    ...
    function migrate() external {
        ...
        require(migrationsDone == false, 'migrations already done');
        ...
        uint token1Balance = token0.balanceOf(address(this));
    ...
    function claimLptokens() external {
        require(unclaimedBalances[msg.sender] >= 0, 'no unclimed balance');
        require(migrationsDone = true, 'Migrations not done yet');
